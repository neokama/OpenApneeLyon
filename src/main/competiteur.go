...
import (
	...
	func (pers Competiteur) check()(bool){
		...
		match, _ = regexp.MatchString("^[\\p{L}- _0-9]*$", pers.equipe )
			if(!match){
				verif =false
				fmt.Println("Erreur! Format du nom d'équipe.")
			}
			if(pers.epreuve1!="sta" && pers.epreuve1 !="spd" && pers.epreuve1!="dwf" && pers.epreuve1!="dnf" && pers.epreuve1!="1650"){
				verif =false
		...
		match, _ = regexp.MatchString("(^[0-9]*)", strconv.Itoa(pers.annonce1))
			...
			if(pers.epreuve2 !="sta" && pers.epreuve2 !="spd" && pers.epreuve2 !="dwf" && pers.epreuve2 !="dnf" && pers.epreuve2 !="1650"){
				verif = false
